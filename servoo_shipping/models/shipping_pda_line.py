...
class ShippingPdaLine(models.Model):
    _name = 'servoo.shipping.pda.line'
    _description = 'Shipping PDA Line'
    _order = 'shipping_pda_id, sequence, id'
    ...
    def _expected_date(self):
        self.ensure_one()
        shipping_pda_date = fields.Datetime.from_string(self.shipping_pda_id.date_pda if self.shipping_pda_id.date_pda and self.shipping_pda_id.state in ['sale', 'done'] else fields.Datetime.now())
        return shipping_pda_date + timedelta(days=self.customer_lead or 0.0)
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            taxes = line.tax_id.compute_all(price, line.shipping_pda_id.currency_id, line.product_uom_qty, product=line.product_id, partner=line.shipping_pda_id.partner_shipping_id)
            line.update({
    ...
    @api.depends('product_id', 'shipping_pda_id.state')
    def _compute_product_updatable(self):
        for line in self:
            if line.state in ['done', 'cancel'] or (line.state == 'sale'):
                line.product_updatable = False
            else:
                line.product_updatable = True

    @api.depends('qty_invoiced', 'product_uom_qty', 'shipping_pda_id.state')
    def _get_to_invoice_qty(self):
        ...
        """
        for line in self:
            if line.shipping_pda_id.state in ['won', 'done']:
    ...
    def _compute_tax_id(self):
        for line in self:
            ...
            fpos = line.shipping_pda_id.fiscal_position_id or line.shipping_pda_id.fiscal_position_id.get_fiscal_position(line.shipping_pda_partner_id.id)
    ...
    def _prepare_add_missing_fields(self, values):
        ...
        if values.get('shipping_pda_id') and values.get('product_id') and any(f not in values for f in onchange_fields):
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            if line.product_id and line.shipping_pda_id.state == 'sale':
                msg = _("Extra line with %s", line.product_id.display_name)
                line.shipping_pda_id.message_post(body=msg)
                # create an analytic account if at least an expense product
                if line.product_id.expense_policy not in [False, 'no'] and not line.shipping_pda_id.analytic_account_id:
                    line.shipping_pda_id._create_analytic_account()
        return lines

    _sql_constraints = [
        ('accountable_required_fields',
            "CHECK(display_type IS NOT NULL OR (product_id IS NOT NULL AND product_uom IS NOT NULL))",
            "Missing required fields on accountable shipping pda line."),
        ('non_accountable_null_fields',
            ...
            "Forbidden values on non-accountable shipping pda line"),
    ]

    def _update_line_quantity(self, values):
        shipping_pdas = self.mapped('shipping_pda_id')
        for shipping_pda in shipping_pdas:
            shipping_pda_lines = self.filtered(lambda x: x.shipping_pda_id == shipping_pda)
            msg = "<b>" + _("The ordered quantity has been updated.") + "</b><ul>"
            for line in shipping_pda_lines:
            ...
            msg += "</ul>"
            shipping_pda.message_post(body=msg)

    def write(self, values):
        if 'display_type' in values and self.filtered(lambda line: line.display_type != values.get('display_type')):
            raise UserError(_("You cannot change the type of a shipping pda line. Instead you should delete the current line and create a new line of the proper type."))
        ...
        if 'done' in self.mapped('shipping_pda_id.state') and any(f in values.keys() for f in protected_fields):
    ...
    invoice_lines = fields.Many2many('account.move.line', 'servoo_shipping_pda_line_invoice_rel', 'shipping_pda_line_id', 'invoice_line_id', string='Invoice Lines', copy=False)
    ...
    product_id = fields.Many2one(
        'product.product', string='Service', domain="[('sale_ok', '=', True), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
    ...
    is_downpayment = fields.Boolean(
        string="Is a down payment", help="Down payments are made when creating invoices from a pda."
        " They are not copied when duplicating a pda.")

    state = fields.Selection(
        related='shipping_pda_id.state', string='Pda Status', copy=False, store=True)
    ...
    def _compute_untaxed_amount_invoiced(self):
        """ Compute the untaxed amount already invoiced from the shipping pda line, taking the refund attached
    ...
    def _compute_untaxed_amount_to_invoice(self):
        """ Total of remaining amount to invoice on the shipping pda line (taxes excl.) as
            ...
            if line.state in ['sale', 'done']:
                ...
                if len(line.tax_id.filtered(lambda tax: tax.price_include)) > 0:
                    ...
                    price_subtotal = line.tax_id.compute_all(
                        price_reduce,
                        currency=line.shipping_pda_id.currency_id,
                        quantity=uom_qty_to_consider,
                        product=line.product_id,
                        partner=line.shipping_pda_id.partner_shipping_id)['total_excluded']
    ...
    @api.depends('product_id', 'shipping_pda_id.date_pda', 'shipping_pda_id.partner_id')
    def _compute_analytic_tag_ids(self):
        for line in self:
            if not line.analytic_tag_ids:
                default_analytic_account = line.env['account.analytic.default'].sudo().account_get(
                    product_id=line.product_id.id,
                    partner_id=line.shipping_pda_id.partner_id.id,
                    user_id=self.env.uid,
                    date=line.shipping_pda_id.date_pda,
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if self.shipping_pda_id.analytic_account_id:
            res['analytic_account_id'] = self.shipping_pda_id.analytic_account_id.id
        if optional_values:
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        comming from a shipping pda line. This method could be override in order to add other custom key that could
        be used in move/po creation.
        """
        return {}

    def _get_display_price(self, product):
        # TO DO: move me in master/saas-16 on servoo.shipping.pda
        ...
        product_context = dict(self.env.context, partner_id=self.shipping_pda_id.partner_id.id, date=self.shipping_pda_id.date_pda, uom=self.product_uom.id)
        base_price, currency = self.with_context(product_context)._get_real_price_currency(product, None, self.product_uom_qty, self.product_uom)
        if currency != self.shipping_pda_id.currency_id:
            base_price = currency._convert(
                base_price, self.shipping_pda_id.currency_id,
                self.shipping_pda_id.company_id or self.env.company, self.shipping_pda_id.date_pda or fields.Date.today())
    ...
    def _update_description(self):
        ...
        product = self.product_id.with_context(
            lang=get_lang(self.env, self.shipping_pda_id.partner_id.lang).code,
        )

        self.update({'name': self.get_shipping_pda_line_multiline_description_sale(product)})
    ...
    def _update_taxes(self):
        ...
        product = self.product_id.with_context(
            partner=self.shipping_pda_id.partner_id,
            quantity=vals.get('product_uom_qty') or self.product_uom_qty,
            date=self.shipping_pda_id.date_pda,
            uom=self.product_uom.id
        )

        self._compute_tax_id()

        if self.shipping_pda_id.partner_id:
            vals['price_unit'] = product._get_tax_included_unit_price(
                self.company_id,
                self.shipping_pda_id.currency_id,
                self.shipping_pda_id.date_pda,
                'sale',
                fiscal_position=self.shipping_pda_id.fiscal_position_id,
                product_price_unit=self._get_display_price(product),
                product_currency=self.shipping_pda_id.currency_id
            )
    ...
    #     if self.shipping_pda_id.partner_id:
    #         product = self.product_id.with_context(
    #             lang=self.shipping_pda_id.partner_id.lang,
    #             partner=self.shipping_pda_id.partner_id,
    #             quantity=self.product_uom_qty,
    #             date=self.shipping_pda_id.date_pda,
    #             uom=self.product_uom.id,
    ...
    #             self.company_id or self.shipping_pda_id.company_id,
    #             self.shipping_pda_id.currency_id,
    #             self.shipping_pda_id.date_pda,
    #             'sale',
    #             fiscal_position=self.shipping_pda_id.fiscal_position_id,
    ...
    def name_get(self):
        ...
        for so_line in self.sudo():
            name = '%s - %s' % (so_line.shipping_pda_id.name, so_line.name and so_line.name.split('\n')[0] or so_line.product_id.name)
            if so_line.shipping_pda_partner_id.ref:
                name = '%s (%s)' % (name, so_line.shipping_pda_partner_id.ref)
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        if operator in ('ilike', 'like', '=', '=like', '=ilike'):
            args = expression.AND([
                args or [],
                ['|', ('shipping_pda_id.name', operator, name), ('name', operator, name)]
            ])
            return self._search(args, limit=limit, access_rights_uid=name_get_uid)
        return super(ShippingPdaLine, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def _check_line_unlink(self):
        ...
        :rtype: recordset servoo.shipping.pda.line
    ...
    def _get_real_price_currency(self, product, rule_id, qty, uom):
        ...
        if rule_id:
            ...
            if pricelist_item.pricelist_id.discount_policy == 'without_discount':
                while pricelist_item.base == 'pricelist' and pricelist_item.base_pricelist_id and pricelist_item.base_pricelist_id.discount_policy == 'without_discount':
                    _price, rule_id = pricelist_item.base_pricelist_id.with_context(uom=uom.id).get_product_price_rule(product, qty, self.shipping_pda_id.partner_id)
                    pricelist_item = PricelistItem.browse(rule_id)
        ...
        if not currency_id:
            ...
            if currency_id.id == product_currency.id:
                ...
                cur_factor = currency_id._get_conversion_rate(product_currency, currency_id, self.company_id or self.env.company, self.shipping_pda_id.date_pda or fields.Date.today())
    ...
    def get_shipping_pda_line_multiline_description_sale(self, product):
        """ Compute a default multiline description for this sales order line.

        In most cases the product description is enough but sometimes we need to append information that only
        exists on the shipping pda line itself.
        e.g:
        - custom attributes and attributes that don't create variants, both introduced by the "product configurator"
        - in event_sale we need to know specifically the sales order line as well as the product to generate the name:
          the product is not sufficient because we also need to know the event_id and the event_ticket_id (both which belong to the shipping pda line).
        """
        return product.get_product_multiline_description_sale() + self._get_shipping_pda_line_multiline_description_variants()

    def _get_shipping_pda_line_multiline_description_variants(self):
        ...
        for ptav in no_variant_ptavs:
            name += "\n" + ptav.with_context(lang=self.shipping_pda_id.partner_id.lang).display_name
        return name
